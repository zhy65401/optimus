...
class QCut(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        if not is_numeric_dtype(X):
            raise TypeError(
                f"Feature `{X.name}` is not a numerical feature, but the qcut strategy is selected."
            )
        _, self.bins = pd.qcut(
            X, q=self.target_bin_cnt, duplicates="drop", retbins=True
        )
...
class SimpleCut(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        if not is_numeric_dtype(X):
            raise TypeError(
                f"Feature `{X.name}` is not a numerical feature, but the simple_cut strategy is selected."
            )
        _, self.bins = pd.qcut(
            X,
            [0, 0.02, 0.04, 0.05, 0.95, 0.96, 0.98, 1],
            duplicates="drop",
            retbins=True,
        )
        self.bins = Common._adjust_bins(self.bins)
...
class ChiMergeCut(BaseEstimator, TransformerMixin):
    def __init__(
        self, target_intervals: int, initial_intervals=False, copy: bool = True
    ):
        """
    ...
    def fit(self, X, y):
        ...
        while len(y) > self.target_intervals:
            ...
            chi2 = np.array(
                [self.chisquare(y[i : i + 2, :]) for i in range(len(y) - 1)]
            )
...
class BestKSCut(BaseEstimator, TransformerMixin):
    ...
    def _stat_bins(self, X, y):
        ...
        df_bins["ks"] = np.abs(
            np.cumsum(df_bins["bad_dist"]) - np.cumsum(df_bins["good_dist"])
        )
    ...
    def cut_bins(self, X, y, left_idx, right_idx):
        ...
        bin_X, bin_y = (
            X[X.between(left, right, inclusive="right")],
            y[X.between(left, right, inclusive="right")],
        )
        ...
        left_bin_total = stat_bin.loc[
            stat_bin["bin"].between(left, best_ks_knot, inclusive=inclusive_mode),
            "bin_cnt",
        ].sum()
        right_bin_total = stat_bin.loc[
            stat_bin["bin"].between(best_ks_knot, right, inclusive="right"), "bin_cnt"
        ].sum()
        if (left_bin_total < self._total * self.min_bin_rate) or (
            right_bin_total < self._total * self.min_bin_rate
        ):
            return []
    ...
    def _evaluate_iv(self, ids):
        ...
        lst_df.extend(
            [self._stat_df.loc[ids[i - 1] + 1 : ids[i]] for i in range(1, len(ids))]
        )
        ...
        if sorted(lst_woe) != lst_woe and sorted(lst_woe, reverse=True) != lst_woe:
            return -1
        lst_iv = (ratio_good - ratio_bad) * np.log(
            (ratio_good + self._eps) / (ratio_bad + self._eps)
        )
        return sum(lst_iv)
    ...
    def fit(self, X, y):
        ...
        if not is_numeric_dtype(inX):
            raise TypeError(
                f"Feature `{inX.name}` is not a numerical feature, but the BestKS strategy is selected."
            )
        self._total = len(inX)
        self.init_bins = sorted(inX.unique())
        if len(self.init_bins) <= self.target_bin_cnt:
            cprint(
                f"[WARNING] {inX.name}: Feature unique number is less than target bin!",
                "yellow",
            )
        ...
        if best_ks_knots_:
            ...
            for target_knot_cnt in range(self.target_bin_cnt, 2, -1):
                ...
                lst_comb = [
                    list(lst_knot_idx)
                    for lst_knot_idx in combinations(
                        range(len(best_ks_knots_)),
                        min(len(best_ks_knots_), target_knot_cnt - 1),
                    )
                ]
                # Control the efficiency. If there are too many combinations, try a smaller target_bins.
                if len(lst_comb) > 10_000:
                    cprint(
                        f"[WARNING] {inX.name}: Too many combination ({len(lst_comb)}) at target_bin_cnt=={target_knot_cnt}. Try with a larger min_bin_rate.",
                        "yellow",
                    )
                    continue
                ...
                if max(lst_iv) > 0:
                    self.bins = np.array(
                        Common._append_bins(
                            [best_ks_knots_[i] for i in lst_comb[np.argmax(lst_iv)]]
                        ),
                        dtype=float,
                    )
                    return self
...
class WOEMerge(BaseEstimator, TransformerMixin):
    ...
    def _pre_merge(self, X):
        ...
        if np.count_nonzero(~mask_others) == 0:
            cprint(
                f"[WARNING] {X.name}: All categories are moved to `others` bin!",
                "yellow",
            )
    ...
    def fit(self, X, y):
        ...
        if self.dtype == "numerical":
            if len(self._optb.splits) == 0:
                ...
                cprint(
                    f"[WARNING] {self.name}: No optimal splits found, using single bin.",
                    "yellow",
                )
                self.bins = [float("-inf"), float("inf")]
            else:
                self.bins = Common._adjust_bins(
                    [min(X.values), *self._optb.splits, max(X.values)]
                )
        else:
            ...
            if hasattr(self._optb, "splits") and len(self._optb.splits) > 0:
                ...
                cprint(
                    f"[WARNING] {self.name}: No optimal splits found, using all unique categories.",
                    "yellow",
                )

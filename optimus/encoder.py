...
class Encoder(BaseEstimator, TransformerMixin):
    ...
    def _split_dataset(self, X, y=None):
        ...
        normal_idc = X.apply(lambda x: (not pd.isna(x)) and (x not in self.missing_values))
    ...
    def _stat_feat(self, X_normal, y_normal, X_missing, y_missing):
        original_cols = [
            'bin_total', 'bin_rate', 'bin_dist',
            'bad', 'bad_rate', 'bad_dist', 'cum_bad_dist', 
            'good', 'good_rate', 'good_dist', 'cum_good_dist', 
            'odds_neg', 'odds_pos', 'ks', 'lift', 'woe', 'iv_bin', 'iv_total',
        ]
        renamed_cols = [
            ['Obs', 'Obs', 'Obs', 'Bad', 'Bad', 'Bad', 'Bad', 'Good', 'Good', 'Good', 'Good', 'OR', 'OR', 'KS', 'Lift', 'WOE', 'IV', 'IV'],
            ['#Obs', '%Obs', 'dist', '#Bad', '%Bad', 'dist', 'cum_dist', '#Good', '%Good', 'dist', 'cum_dist', 'neg', 'pos', '', '', '', 'bin', 'feature']
        ]
        ...
        gp_missing = df_missing.groupby('bins', observed=True).agg(
            bin_total=('label', 'count'),
            bad=('label', lambda x: sum(x)),
            good=('label', lambda x: len(x)-sum(x))
        ).sort_index().reset_index()
        ...
        res['woe'] = res['bins'].map(lambda x: self.bin_info[X_normal.name].get(x, np.mean(list(self.bin_info[X_normal.name].values())))).astype(float) 
        res['iv_bin'] = res['woe'] * (res['good_dist'] - res['bad_dist'])
        res['iv_total'] = res['iv_bin'].sum()
        res.index = pd.MultiIndex.from_tuples([(X_normal.name, b) for b in res.bins], names=('feature_name', 'bins'))
    ...
    def _get_woe(self, X_normal, y_normal, X_missing, y_missing):
        ...
        feat_bin_info = {}
        feat_bin_info.update(pd.concat([X_normal, y_normal], axis=1).rename({X_normal.name: 'bin', y_normal.name: 'label'}, axis=1).groupby('bin', observed=True).agg(
            woe=('label', lambda x: np.log(((len(x)-sum(x))/total_good+self._eps) / (sum(x)/total_bad+self._eps)))
        ).sort_values('woe', ascending=False).to_dict()['woe'])
        feat_bin_info.update(pd.concat([X_missing, y_missing], axis=1).rename({X_missing.name: 'bin', y_missing.name: 'label'}, axis=1).groupby('bin', observed=True).agg(
            woe=('label', lambda x: np.log(((len(x)-sum(x))/total_good+self._eps) / (sum(x)/total_bad+self._eps)))
        ).sort_values('woe', ascending=False).to_dict()['woe'])
        ...
        for feat, bins_woe in self.bin_info.items():
            if ('__N.A__' not in bins_woe) and (-990000 not in bins_woe):
                ...
                if not normal_woes:
                    ...
                    raise ValueError(f'Unknown treat_missing strategy `{self.treat_missing}`, expected one of (`mean`, `min`, `max`, `zero`)')
                at_least_bin = -990000 if self._feat_types[feat] == 'numerical' else '__N.A__'
    ...
    def fit(self, X, y, **kwargs):
        ...
        if diff_cols:
            raise KeyError(f"{len(diff_cols)} columns haven't decided the binning strategy yet!")
        idx = 0
        for feat, bin_strategy in self.spec.items():
            ...
            if isinstance(bin_strategy, list):
                ...
                nu = X_normal.nunique()
                chimerge = ChiMergeCut(target_bin_cnt, initial_intervals=100 if nu > 100 else False)
                ...
                if len(bins) > 10:
                    cprint(f"[WARN] {feat} have too many categories, suggest to bin before!", "yellow")
